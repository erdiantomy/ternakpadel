...
function ScorerOverlay({ S, A }) {
  ...
  return (
    <div style={{ position: "absolute", inset: 0, zIndex: 70, background: "var(--bg)", display: "flex", flexDirection: "column", animation: "tpFade .15s" }}>
      <Row style={{ justifyContent: "space-between", padding: "14px 16px" }}>
...
function RankingsScreen({ S }) {
  ...
  return (
    <Col gap={12} style={{ padding: "calc(14px * var(--sp)) 16px 90px" }}>
      ...
      <Seg options={["Weekly", "Monthly", "Season", "All time"]} value={period} onChange={setPeriod}></Seg>
      ...
      <Row gap={10} style={{ alignItems: "flex-end", justifyContent: "center", padding: "8px 0 2px" }}>
        {order.map((p, i) => (
          <Col key={p.id} gap={5} style={{ alignItems: "center", flex: 1 }}>
            <Ava ini={p.initials} d={i === 1 ? 46 : 38} ring={i === 1}></Ava>
      ...
      <Card pad={8}>
        {rest.map((p, i) => (
          <Row key={p.id} gap={10} style={{
            ...
            <Ava ini={p.initials} d={28} ring={p.me}></Ava>
            <Body size={13.5} bold={p.me} style={{ flex: 1 }}>{p.name}{p.me ? " (you)" : ""}</Body>
...
Object.assign(window, { MatchesScreen, ScorerOverlay, RankingsScreen });
