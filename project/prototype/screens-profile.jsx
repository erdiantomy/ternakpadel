// Profile / career screen, share-card overlay, onboarding, create-match sheet.
...
function ProfileScreen({ S, A }) {
  ...
  return (
    <Col gap={12} style={{ padding: "calc(14px * var(--sp)) 16px 90px" }}>
      <Row gap={12}>
        <Ava ini={me.initials} d={56} ring></Ava>
      ...
      <Card>
        ...
        <div style={{ marginTop: 8 }}>
          <Spark vals={S.rankHistory.map((v) => -v)} w={330} h={64} stroke={2.5}></Spark>
        </div>
      ...
      <Col gap={0}>
        {S.timeline.map((t, i) => (
          <Row key={t.id} gap={11} style={{ alignItems: "flex-start", position: "relative", paddingBottom: i === S.timeline.length - 1 ? 0 : 14 }}>
            {i !== S.timeline.length - 1 && <div style={{ position: "absolute", left: 14, top: 30, bottom: 0, width: 2, background: "var(--line)" }}></div>}
            <div style={{
  ...
  return (
    <div onClick={A.closeShare} style={{
      ...
      <div onClick={(e) => e.stopPropagation()} style={{
        ...
        padding: 18, animation: "tpPop .3s cubic-bezier(.2,1.3,.4,1)",
      }}>
        <div style={{ position: "absolute", right: -44, top: -44, width: 150, height: 150, borderRadius: "50%", background: "var(--accent)", opacity: 0.22 }}></div>
        <div style={{ position: "absolute", left: -30, bottom: 60, width: 90, height: 90, borderRadius: "50%", border: "2px solid var(--accent)", opacity: 0.25 }}></div>
...
// ---------- Create match sheet (admin / FAB) ----------

function CreateSheet({ S, A }) {
...
function Onboarding({ A }) {
  ...
  return (
    <div style={{ position: "absolute", inset: 0, zIndex: 100, background: "var(--bg)", display: "flex", flexDirection: "column", padding: "18px 20px 24px" }}>
      {step === 0 && (
      ...
      {step >= 2 && step <= 5 && (() => {
        ...
        return (
          <Col gap={12} style={{ flex: 1, paddingTop: 16 }}>
            <Row gap={5}>
              {qs.map((_, i) => (
                <div key={i} style={{ flex: 1, height: 4, borderRadius: 3, background: i <= step - 2 ? "var(--accent)" : "var(--line)" }}></div>
              ))}
            </Row>
      ...
      {step < 6 && (
        <button onClick={A.finishOnboarding} style={{ position: "absolute", top: 16, right: 18, background: "none", border: "none", color: "var(--text2)", fontFamily: "var(--font-body)", fontSize: 13, cursor: "pointer" }}>Skip demo →</button>
      )}
    </div>
  );
}

Object.assign(window, { ProfileScreen, ShareOverlay, CreateSheet, Onboarding });
