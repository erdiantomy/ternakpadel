-- ============================================================================
-- Demo Mode visibility: keep demo activity private until the admin promotes it.
--
-- Demo Mode writes REAL data (scores feed the real leaderboard, players keep
-- their id). The downside is that demo events would otherwise show up in the
-- player app — and the app treats the first status='live' event as THE live
-- event for everyone (LiveApp.jsx), so scoring a demo would hijack players'
-- screens and spam the feed.
--
-- Fix: an is_demo flag on EVENTS (and the feed posts they generate). The player
-- app filters these out; admins see everything. Points / identity are NOT
-- flagged, so continuity is preserved. "Go live" simply flips is_demo = false.
--
-- Safe to run more than once.
-- ============================================================================

alter table public.events     add column if not exists is_demo boolean not null default false;
alter table public.feed_posts add column if not exists is_demo boolean not null default false;

create index if not exists events_is_demo_idx on public.events (is_demo);

-- finish_match (supersedes 0010): identical, except the result feed post inherits
-- the event's is_demo so demo results stay hidden from players until promotion.
