-- 0003: complete idempotent setup — full schema + superadmin.
...
select 'Season 3', '2026-04-01', '2026-08-31', true
...
create index if not exists matches_event_round on public.matches (event_id, round);
...
drop policy if exists "points readable" on public.player_points;
create policy "points readable" on public.player_points for select to authenticated using (true);
drop policy if exists "rank history readable" on public.rank_history;
...
drop policy if exists "player badges readable" on public.player_badges;
...
drop policy if exists "host updates own events" on public.events;
create policy "host updates own events" on public.events for update to authenticated using (created_by = auth.uid());

drop policy if exists "rosters readable" on public.event_players;
...
drop policy if exists "players & host score" on public.matches;
...
drop policy if exists "feed readable" on public.feed_posts;
create policy "feed readable" on public.feed_posts for select to authenticated using (true);
drop policy if exists "post as self" on public.feed_posts;
...
drop policy if exists "like as self" on public.feed_likes;
create policy "like as self" on public.feed_likes for insert to authenticated with check (player_id = auth.uid());
drop policy if exists "unlike as self" on public.feed_likes;
...
do $$ begin alter publication supabase_realtime add table public.event_players; exception when duplicate_object then null; end $$;
do $$ begin alter publication supabase_realtime add table public.feed_posts; exception when duplicate_object then null; end $$;
do $$ begin alter publication supabase_realtime add table public.feed_likes; exception when duplicate_object then null; end $$;
do $$ begin alter publication supabase_realtime add table public.payments; exception when duplicate_object then null; end $$;


-- ============================================================ heal existing profiles
alter table public.profiles add column if not exists username   text;
alter table public.profiles add column if not exists full_name  text not null default '';
alter table public.profiles add column if not exists phone      text;
alter table public.profiles add column if not exists skill      text not null default 'Intermediate';
alter table public.profiles add column if not exists side       text not null default 'Left';
alter table public.profiles add column if not exists play_freq  text;
alter table public.profiles add column if not exists goal       text;
alter table public.profiles add column if not exists city       text not null default 'Jakarta';
...
-- allow admin announcements in the feed
...
alter table public.feed_posts add constraint feed_posts_kind_check
  check (kind in ('result','rank','badge','join','announcement'));
...
create policy "admin manage payments" on public.payments for all to authenticated using (public.is_admin()) with check (public.is_admin());
...
create policy "admin manage rosters" on public.event_players for all to authenticated using (public.is_admin()) with check (public.is_admin());
drop policy if exists "admin manage feed" on public.feed_posts;
create policy "admin manage feed" on public.feed_posts for all to authenticated using (public.is_admin()) with check (public.is_admin());
...
select id, true from auth.users where email = 'biz@tomspadel.com'
