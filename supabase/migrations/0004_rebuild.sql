-- 0004: REBUILD — drops the app tables (old padel-app collisions) and recreates
-- the correct schema, then installs superadmin + grants you. Re-runnable.
-- NOTE: clears app data (test signups). Google logins persist; users re-onboard.

drop view     if exists public.event_standings cascade;
...
drop table if exists public.player_points cascade;
...
-- backfill profiles for everyone who already signed in (trigger only fires on NEW signups)
...
where id in (select id from auth.users where email = 'biz@tomspadel.com')
