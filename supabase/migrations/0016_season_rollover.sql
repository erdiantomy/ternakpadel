-- ============================================================================
-- Season rollover. Season 3 ends 2026-08-31; without a follow-up season row
-- (and something flipping is_current) every finish_match raises from Sep 1 —
-- player_points.season_id is NOT NULL, so a null season aborts the insert.
--
--   1. Seed Season 4 (2026-09-01 → 2026-12-31), not current yet.
--   2. A daily pg_cron job keeps seasons.is_current aligned with the calendar,
--      so the app (which reads is_current everywhere) rolls over by itself.
--   3. finish_match raises a CLEAR error when no season covers today, instead
--      of a null-constraint failure deep in the points upsert.
--
...
select 'Season 4', date '2026-09-01', date '2026-12-31', false
where not exists (select 1 from public.seasons where name = 'Season 4');

-- 2. calendar-driven is_current ----------------------------------------------
create or replace function public.roll_current_season()
returns void language sql security definer set search_path = public as $$
  update public.seasons set is_current = (current_date between starts_on and ends_on)
  where is_current is distinct from (current_date between starts_on and ends_on);
$$;
revoke all on function public.roll_current_season() from public, anon, authenticated;

select public.roll_current_season();

-- daily at 17:05 UTC = 00:05 Asia/Jakarta, right after the local date changes
do $$ begin perform cron.unschedule('season-rollover'); exception when others then null; end $$;
select cron.schedule('season-rollover', '5 17 * * *', $$ select public.roll_current_season() $$);

-- 3. finish_match: explicit guard (supersedes 0011 — identical except the
--    season lookup now raises a clear error instead of failing on a null
--    season_id inside the first player_points insert).
...
begin
  ...
  if season is null then
    -- fall back to the season whose date range covers today (the rollover cron
    -- normally keeps is_current in sync; this covers a missed run)
    select id into season from public.seasons
      where current_date between starts_on and ends_on
      order by id desc limit 1;
  end if;
  if season is null then
    raise exception 'no active season — add a public.seasons row covering today';
  end if;
